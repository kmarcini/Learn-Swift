...
//  Arrays_And_Loops
//
//  Created by Kevin Marciniak on 5/11/17.
...
func printResults(forIssue: String, withVotes: [Bool]) {
    ...
    finalStatement += " \(yes) yes, \(no) no"
    print(finalStatement)
}

printResults(forIssue: "Should we change the mascot?", withVotes:shouldMascotChangeVotes)
printResults(forIssue: "Should we install a coffee vending machine?", withVotes:shouldInstallCoffeeVendingMachineVotes)
printResults(forIssue: "Should we have more poll options?", withVotes:shouldHaveMorePollOptionsVotes)
print()

let milesBiked = [4, 8, 2, 10, 0, 0, 3, 6, 7, 8, 11, 0, 0, 20, 2, 3, 0, 0, 0, 8]

func onTarget(_ actual: Int) -> String {
    let target = 8
    
    if actual >= (target - 1) && actual <= (target + 1) {
        return "On target!"
    } else if actual < (target - 1) {
        return "Below target!"
    } else {
        return "Above target, don't try too hard!"
    }
}

for dailyMilesBiked in milesBiked {
    print(onTarget(dailyMilesBiked))
}
